...
function SkillBox({
  ...
  const textMat = useNeonMaterial(
    new Color(1, 1.25, 1.25),
  ...
  const basePositionY = useMemo(
    () => Math.floor(index / 4) * (WIDTH + GAP) + ((WIDTH + GAP) / 2 + GAP),
    [index, GAP, WIDTH],
  );

  useFrame(({ clock }) => {
    ref.current.position.y =
      basePositionY + Math.sin(clock.getElapsedTime() * multiplier) / 6;
  });
  ...
  return (
    <group
      ...
      position-y={
        Math.floor(index / 4) * (WIDTH + GAP) + ((WIDTH + GAP) / 2 + GAP / 2)
      }
      ref={ref}
      {...props}
    >
